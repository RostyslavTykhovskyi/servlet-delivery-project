...
<div class="container" style="width: 500px; margin-top: 50px; margin-bottom: 50px;">
    <form action="${pageContext.request.contextPath}/order" method="POST">
        ...
        <c:if test="${not empty requestScope.cost}">
            <div class="fs-3 mb-2">
                <fmt:message key="order.calculatedCost"/>: ${requestScope.cost} <fmt:message key="currency"/>
            </div>
        </c:if>
        <fmt:message key="button.calculate" var="buttonCalculate"/>
        <input class="btn btn-primary" type="submit" name="calculate" value="${buttonCalculate}">
        <c:if test="${not empty sessionScope.username}">
            <fmt:message key="button.order" var="buttonOrder"/>
            <input class="btn btn-primary" type="submit" name="makeOrder" value="${buttonOrder}">
        </c:if>
    </form>
</div>

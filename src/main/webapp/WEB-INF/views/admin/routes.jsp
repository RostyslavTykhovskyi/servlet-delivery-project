...
<div class="container-lg d-flex flex-column align-items-center">
    ...
    <c:if test="${requestScope.routes.size() > 0}">
        <table class="table table-hover align-middle" style="width: 800px;">
            ...
            <tr>
                <th scope="col">
                    <a class="link-dark text-decoration-none" href="${pageContext.request.contextPath}/admin/routes?page=${requestScope.page}&sortField=route_id&sortDirection=${requestScope.reverseSortDir}">
                        Id${requestScope.sortField != 'route_id' ? '' : requestScope.sortDirection == 'DESC' ? '▾' : '▴'}
                    </a>
                </th>
                <th scope="col">
                    <a class="link-dark text-decoration-none" href="${pageContext.request.contextPath}/admin/routes?page=${requestScope.page}&sortField=departure_point&sortDirection=${requestScope.reverseSortDir}">
                        <fmt:message key="route.departure"/>${requestScope.sortField != 'departure_point' ? '' : requestScope.sortDirection == 'DESC' ? '▾' : '▴'}
                    </a>
                </th>
                <th scope="col">
                    <a class="link-dark text-decoration-none" href="${pageContext.request.contextPath}/admin/routes?page=${requestScope.page}&sortField=arrival_point&sortDirection=${requestScope.reverseSortDir}">
                        <fmt:message key="route.arrival"/>${requestScope.sortField != 'arrival_point' ? '' : requestScope.sortDirection == 'DESC' ? '▾' : '▴'}
                    </a>
                </th>
                <th scope="col">
                    <a class="link-dark text-decoration-none" href="${pageContext.request.contextPath}/admin/routes?page=${requestScope.page}&sortField=length&sortDirection=${requestScope.reverseSortDir}">
            ...
            <c:forEach var="route" items="${requestScope.routes}">
                <tr>
                    ...
                    <td>${route.length}</td>
                    <td>
                        <a class="btn btn-primary" href="${pageContext.request.contextPath}/admin/routes/${route.id}">Update</a>
                    </td>
                </tr>
            </c:forEach>
            </tbody>
        ...
        <nav>
            <ul class="pagination">
                <li ${requestScope.page == 1 ? 'class="page-item disabled"' : 'class="page-item"'}>
                    <a class="page-link"
                       href="${pageContext.request.contextPath}/admin/routes?page=${requestScope.page - 1}&sortField=${requestScope.sortField}&sortDirection=${requestScope.sortDirection}"
                       aria-label="Previous">
                ...
                <c:forEach begin="1" end="${requestScope.pageNumber}" var="i">
                    <li ${i == requestScope.page ? 'class="page-item active"' : 'class="page-item"'}>
                        <a class="page-link"
                           href="${pageContext.request.contextPath}/admin/routes?page=${i}&sortField=${requestScope.sortField}&sortDirection=${requestScope.sortDirection}">${i}</a>
                    </li>
                </c:forEach>
